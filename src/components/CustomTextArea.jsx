...
const CustomTextArea = ({ hasPermission = true, matchData = [], validateFn, handleApply, labelConfig }) => {
    ...
    const convertToTags = textArea => {
        ...
        const tagList = [...new Set([...content?.replaceAll('\n', ' ')?.replaceAll(/\,/g, ' ')?.replaceAll(/\，/g, ' ')?.trim()?.split(/\s+/)])]
        const tempValid = [],
            tempInValid = []
        tagList.forEach(item => {
            const tag = item?.trim();
            tag && ((validateFn ? validateFn(tag, matchData) : true) ? tempValid : tempInValid).push(tag)
        })
        setValid(tempValid)
    ...
    const deleteTag = child => {
        ...
        const [cls, tag] = child.id.split('~'),
            ...
            idx = (deleteType ? tempInValid : tempValid).findIndex(item => item === tag),
    ...
    return (
        <>
            {hasPermission && (
                <Tooltip placement="bottomLeft" title={labelConfig.title} arrowPointAtCenter>
            ...
            <Modal
                ...
                okText='Apply'
                cancelButtonProps={{ type: 'text', size: 'middle', style: { display: 'none' } }}
                ...
                <>
                    ...
                    <div className='bulk-add-match'>
                        ...
                        <div className='bulk-add-clear-all' onClick={handleClearAll}>Clear All</div>
                    </div>
                    {inValid.length > 0 && unfoundCount === 0 && <div className='bulk-add-error-info'>{labelConfig.errorMessage}</div>}
                </>
            </Modal>
        </>
    );
...
CustomTextArea.defaultProps = {
    labelConfig: {
        ...
        errorMessage: 'This value exceeds limitation or is invalid. Please try again',
    },
